...
Los atributos, todos strings, son `nombre`, `apellidos`, `dni`, `alias` y `posición`. Además se definen los _getters_ y los _setters_ de todos sus atributos.

### Posicion (`Posicion.java`)
Se trata del archivo donde se define el objeto `Posiciones` y se definen estas mismas.  
El archivo comienza definiendo el número máximo de cada posición junto a un _getter_ del nombre de la posición como de el número máximo por posición o del número de jugagores actuales.  
